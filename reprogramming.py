...
class ReprogrammingFuntion(nn.Module):
    ...
    def forward(self, sentence_batch):
        ...
        for patch_idx in range(min(_L, self.num_patches)):
...
def evaluate(dataloader, vit_model, reprogrammer, tb_writer, iter_no, max_batches = None):
    total_correct = 0.0
    total_examples = 0.0
    reprogrammer.eval()
    for bidx, batch in enumerate(dataloader):
        ...
        logits = vit_model(programmed_img)
...
def main():
    ...
    train_dataset = train_dataset.map(lambda e: label_mapper(e, args.text_dataset), batched=True)
    ...
    val_dataset = val_dataset.map(lambda e: label_mapper(e, args.text_dataset), batched=True)
    ...
    reprogrammer = ReprogrammingFuntion(vocab_size, train_hps['img_patch_size'], train_hps['img_size'])
    reprogrammer.cuda()
    ...
    for epoch in range(train_hps['num_epochs']):
        for bidx, batch in enumerate(train_loader):
            ...
            if iter_no % train_hps['evaluate_every'] == 0:
                print("Evaluating")
                metrics = evaluate(val_loader, vit_model, 
